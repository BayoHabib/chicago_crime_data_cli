...
import pytest
from chicago_crime_downloader import RunConfig, HttpConfig, run_windowed_mode
...
def test_stop_requested_breaks_window(tmp_path):
    ...
    try:
        cfg = RunConfig("daily", tmp_path, "csv", 50000, None, "2020-01-01", "2020-01-01", None, None)
        http = HttpConfig()
        headers = {}

        # should exit cleanly without creating dirs
        run_windowed_mode(cfg, http, headers, None, [(date(2020,1,1), date(2020,1,1), "2020-01-01")], "daily")
